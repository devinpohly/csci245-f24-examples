...
public class ClockProgram extends JFrame implements ClockView {
    ...
    InvocationTargetException {
        ...
        upper.add(ampm);
        final JComboBox modeList = new JComboBox(boxOptions);
        modeList.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent ae) {
                    if (modeList.getSelectedItem().equals("Time"))
                        model.menuSelection(0);
                    else if (modeList.getSelectedItem().equals("Set hours"))
                        model.menuSelection(1);
                    else if (modeList.getSelectedItem().equals("Set minutes"))
                        model.menuSelection(2);
                    else if (modeList.getSelectedItem().equals("Set seconds"))
                        model.menuSelection(3);
                    else if (modeList.getSelectedItem().equals("Set AM/PM"))
                        model.menuSelection(4);
                }
            });
        upper.add(modeList);
        add(upper);
